            ...
            <div class="lt-overflow-hidden lt-border lt-border-gray-200 lt-rounded-lg">
                <table class="lt-min-w-full lt-divide-y lt-divide-gray-200">
                    ...
                    @foreach($associations->filter(fn($product) => $product['inverse'] == $showInverseAssociations) as $index => $product)
                        <tr
                                class="lt-bg-white even:lt-bg-gray-50"
                        ...
                        >
                            ...
                            <x-l-tables::cell>
                                <a
                                        href="{{ route('hub.products.show', $product['target_id']) }}"
                                        class="lt-text-blue-600 hover:underline"
                                >
                                    {{ $product['name'] }}
                                </a>
                            </x-l-tables::cell>
                            ...
                            <x-l-tables::cell>
                                <button
                                        type="button"
                                        wire:click.prevent="removeAssociation({{ $index }})"
